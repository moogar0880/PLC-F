...
fun test5 () = let val seq = permutation []
               in 
                   next(seq) = SOME [] andalso next(rest(seq)) = NONE
               end
fun test6 () = let val seq = !singleSeq
               in 
                   next(seq) = SOME [1] andalso next(rest(seq)) = NONE
               end
fun test7 () = let val seq = !smallSeq
               in
                   contains r1 (next (rest (rest (rest seq))))
               end
fun test8 () = let val seq = !smallSeq
               in
...
fun test9 () = let val seq = !largeSeq
                        in
                            let fun f s [] = true
                                  | f s l = 
                                    if (next s) <> NONE then
...
fun test10 () = let val seq = !smallSeq
                        in
                            let fun f s acc = 
                                    if next(s) <> NONE then
...
fun test15 () = let val f = time (integralMem (polynomial coeff1))
                in
                    ((f 0.0 100.0) > (f 0.0 100.0))
                end
fun test16 () = let val f1 = integralMem parabola
...
val allTests =
    [(1, "test1 create a small sequence [1...3]", test1),
     ...
     (1, "test12 integral of y = x^2 from 0.0 to 10.0", test12),
     (1, "test13 integral of y = x from 0.0 to 5.0", test13),
     (1, "test14 integral of constant from 1.7 to 4.5", test14),
     (1, "test15 integralMem faster on second run with 21st degree polynomial from 0.0 to 100.0", test15),
     (1, "test16 integralMem on two different functions won't cause interference", test16),
     (1, "test17 integralMem faster on second run with 70th degree polynomial from 0.0 to 300.0", test17)]
