 ...
 * Time: 11:19 PM
 */
public class VacuumWorld implements World {
    public int maxX, maxY;
    private VState start;
     ...
     * @param dirts - array of spaces containing dirt needing to be vacuumed
     ...
     */
    public VacuumWorld(Point start, Point[] dirts, Point[] blocked, int maxX, int maxY){
        if( start == null || dirts == null || blocked == null || maxX < 0 || maxY < 0) {
            throw new IllegalArgumentException();
        }
        else{
            this.start   = new VState(null, start, null, null);
            this.dirt    = new ArrayList<Point>(Arrays.asList(dirts));
     ...
     */
    public class VState implements World.State{
        private Point agentLoc, goal;
        private VState parent;
         ...
         */
        private VState( VState par, Point p, Direction lastMove, Point g ){
            parent  = par;
            agentLoc = p;
            goal = g;
         ...
         */
        public VacuumWorld.VState[] getChildren(){
            ArrayList<VState> tmp = new ArrayList<VState>();
            ...
            for( Direction d: directions ){ // assure each are valid children
                ...
                if( isValidChild(p) ){
                    tmp.add(new VState(this, p, d, goal));
                    arrayCounter++;
                }
            }
            Collections.shuffle(tmp);
            VState[] children = tmp.toArray(new VState[arrayCounter]);
            return children;
        }
